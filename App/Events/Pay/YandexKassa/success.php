...
} else if ($label[1] == 'buynolimit') {
    $chatid = $label[0];
    if ($amount >= $label[2]) {
        $db->setPremium($chatid, $label[3]);
        $db->setUserWatermark($chatid, '1');
        $msg->sendHTML(ADMIN_ID, "<a href='tg://user?id={$label[0]}'>Пользователь</a> оплатил Безлимит до " .
            date('d-m-Y H:i') . ", цена: $amount (Yandex)");

        $response = $json['text']['безлимит оплачен'];
        $response = str_replace('{date}', date('d-m-Y H:i', $label[3]), $response);
        $msg->replyHTML($response);
    }
}
